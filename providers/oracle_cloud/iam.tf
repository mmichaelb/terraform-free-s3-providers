resource "oci_identity_compartment" "main" {
  ...
  description    = "The compartment for the bucket."
...
resource "oci_identity_policy" "access_write" {
  name           = "${var.bucket_name}-access-write"
  description    = "The IAM policy the application will use to write to the bucket."
  compartment_id = oci_identity_compartment.main.id
  statements     = [
    "Allow group ${oci_identity_group.access.name} to manage objects in compartment ${oci_identity_compartment.main.name} where all { target.bucket.name='${var.bucket_name}', request.permission='OBJECT_CREATE' }"]
}
